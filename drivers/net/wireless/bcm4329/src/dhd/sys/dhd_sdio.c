...
#ifdef JRD
extern int wifi_call_resume(void);
#endif
...
/* eugene ++ */
		if(ret){
			printk("\n%s: dhdsdio_clkctl(), ret= %d failed,(CLK_AVAIL)++++SLEEPING!! \n", __FUNCTION__, ret);
...
/* eugene ++ */
		if(ret){
			printk("\n%s: dhdsdio_clkctl(), ret= %d failed, (CLK_SDONLY)++++SLEEPING!! \n", __FUNCTION__, ret);
			dhdsdio_clkctl(bus, CLK_SDONLY, FALSE);
		}
/* eugene -- */
...
/* eugene ++ */
		if(ret){
			printk("\n%s: dhdsdio_clkctl(), ret= %d failed, (CLK_AVAIL)++++AWAKE!! \n", __FUNCTION__, ret);
			dhdsdio_clkctl(bus, CLK_AVAIL, FALSE);
		}
/* eugene -- */
...
/* eugene ++ */
		if(ret){
			printk("\n%s: dhdsdio_clkctl(), ret= %d failed, (CLK_SDONLY)++++AWAKE!! \n", __FUNCTION__, ret);
			dhdsdio_clkctl(bus, CLK_SDONLY, FALSE);
		}
/* eugene -- */
...
dhd_enable_oob_intr(struct dhd_bus *bus, bool enable)
{
	...
	/* Turn off our contribution to the HT clock request */
/* eugene add ++ */
	OSL_DELAY(20000);	//eugene add for swoob_polling mode dhd_clkctl timeout !!
/* eugene add -- */

// Try to mark dhdsdio_clkctl(bus, CLK_SDONLY, FALSE) workable 20110429
	ret=dhdsdio_clkctl(bus, CLK_SDONLY, FALSE);

/* eugene add ++ */
	if(ret){
		printk("\n%s: dhdsdio_clkctl(), ret= %d failed, try againg!! \n", __FUNCTION__, ret);
		dhdsdio_clkctl(bus, CLK_SDONLY, FALSE);
	}
/* eugene add -- */
